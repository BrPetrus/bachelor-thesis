...
we concluded that the gradient thresholdolding algorithm provides better results.
...
apart from the installation of the required packages.
...
\begin{itemize}
    ...
    \item \texttt{maximum\_projection\_to} (integer): Along with the previous
        parameter, specify which layers are used in the maximum projection.
...
The primary goal of this thesis was to develop pipeline for automatic segmentation of membrane-stained image data from fluorescent microscopy. This is done to make it easier for researchers to analyse the behaviour of cells, by decreasing the time required to segment them. This program was supposed to be packaged in an easy to use form with exemplary usage documented.

The thesis was documented 
