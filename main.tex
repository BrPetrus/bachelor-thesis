...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{resources/flood\_fill.png}
        \caption{}
    ...
    (\subref{fig:fill-map-zc}) the result of the zero-crossing algorithm is
    shown. Then the flood filling mechanism is run
    (\subref{fig:fill-map-flood-fill}) and finally the mask separating cells and
    background is shown (\subref{fig:fill-map-mask}).}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{resources/labeled\_regions.png}
        \caption{}
        \label{fig:dist-regions}
    \end{subfigure}
    \caption{In (\subref{fig:dist-processed-mask}) the post-processed mask is
    shown. Then the distance transform is applied (\subref{fig:dist-map}), and
    finally the watershed algorithm is run (\subref{fig:dist-regions}).}
...
with the real segmentations. In Figure \ref{fig:evaluation_basic}, I only used two colours
...
$$|R_i \cap P_j| > \alpha \cdot |R_i| \Leftrightarrow R_i \text{ and } P_j \text{ form a pair}$$
...
\end{figure}
The first measure evaluates the accuracy of the predicted segmentation coincides
...
The final results are reported in Table \ref{tab:scores} and the used
configurations in Table \ref{table:configurations}. Every row begins with the
filename and provides various scores I achieved with the program. As can be seen
from the table, certain images got better results than others.

In general, the program has slightly higher recall than precision, meaning that the
...
real cell, it segments it reasonably well. In the last row of Table \ref{tab:scores}, a
simple average of all the metrics are shown with the standard deviation. In
Figure \ref{fig:comparison-1} and in Figure \ref{fig:comparison-2} the
comparison between the program's prediction and the expert's labelling is shown.
In case of the second figure, the program has a lower $\bar{J}$ because several
of the cells were not found by the program, due to significant noise in the
membrane-stained channel. On the other hand, in Figure \ref{fig:comparison-1}
the effect of noise was more suppressed, and the program found more of the
cells, leading to higher $\bar{J}$.

\begin{figure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./resources/t004-roi-10-375-predtiction.png}
        \caption{}
        \label{fig:comparison-pred-1}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./resources/t004-roi-10-375-truth.png}
        \caption{}
        \label{fig:comparison-truth-1}
    \end{subfigure}
    \caption{Prediction (\subref{fig:comparison-pred-1}) for the file
    \texttt{t004-roi-10} and the expert's labelling
    (\subref{fig:comparison-truth-1})}
    \label{fig:comparison-1}
\end{figure}
\begin{figure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./resources/t008-roi-6-180-prediction.png}
        \caption{}
        \label{fig:comparison-pred-2}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./resources/t008-roi-6-180-truth.png}
        \caption{}
        \label{fig:comparison-truth-2}
    \end{subfigure}
    \caption{Prediction (\subref{fig:comparison-pred-2}) for the file
    \texttt{t004-roi-6} and the expert's labelling
    (\subref{fig:comparison-truth-2})}
...
\begin{table}
    \hspace*{-0.5cm}\begin{tabular}{|| c|c|c|c|c|c|c ||}
        ...
        t004-roi-10&0.40&0.61&0.73&0.83&0.65&9.22\\
        ...
        \midrule
        Averages&$0.38$&$0.61$&$0.54$&$0.51$&$0.63$&$6.13$\\
        Standard dev.&$0.11$&$0.037$&$0.12$&$0.16$&$0.16$&$2.29$\\
        \bottomrule
    \end{tabular}
...
The last column in Table \ref{tab:scores} indicates the average time in seconds taken to run the program --- on my computer with the Ryzen 5700U notebook processor --- on
...
specimen. In this case, the solution is quite easy; after manually cropping the
bottom part
