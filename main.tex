...
\cite{hipr-mexican-hat}. 

The Figure \ref{fig:filtering-noise} demonstrates this
effect. By appyling a small amount of noise, the original signal is not
significantly distorted, but hardly anything can be deduced from the second
derivative. However, if Gaussian smoothing is applied first, the second
derivative gets a lot clearer. 
...
Subfigure \ref{fig:zc}. Here another valuable property of the zero-crossing
algorithm can be seen. The produced edges are thin, and form closed contours.
...
the backgground. In principle any metric can be used, but the Euclidean distance
